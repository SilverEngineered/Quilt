...
def perform_pca(keep_digits=list(range(8)), pca_dims=32):
    path = os.path.join("..", "data", "cifar-10-batches-py", "data_batch_1")
    data = unpickle(path)
    images = data[b'data']
    labels = data[b'labels']
    '''x_train_path = os.path.join(path, "full_data", "x_train.npy")
    x_test_path = os.path.join(path, "full_data", "x_test.npy")
    y_train_path = os.path.join(path, "full_data", "y_train.npy")
    y_test_path = os.path.join(path, "full_data", "y_test.npy")
    ...
    pcas = []
    pca = PCA(32)
    pca.fit(preprocessing.normalize(images))
    print(pca.explained_variance_ratio_.cumsum())
    exit()
    x_train_transformed = pca.transform(preprocessing.normalize(images))
...
def perform_pca_new(keep_digits=[0,1,2,3,4,5,6,7], pca_dims=32):
    ...
    data = unpickle(path)
    images = data[b'data']
    labels = np.array(data[b'labels'],dtype=float)
    x_train = images[2000:]
    y_train = labels[2000:]
    x_test = images[:2000]
    y_test = labels[:2000]
    ...
    for i in train_amplitudes:
        ...
        b1.append(i[1])
        b2.append(i[2])
    y_path = os.path.join("..", "data", "splits_cifar", "b0")
    np.save(y_path, b0)
    y_path = os.path.join("..", "data", "splits_cifar", "b1")
    np.save(y_path, b1)
    y_path = os.path.join("..", "data", "splits_cifar", "b2")
    np.save(y_path, b2)
    #-----------------------------------------------------------------------------------------
    split_0_y = []
    for index, i in enumerate(train_amplitudes):
        if i[0] ==-1 and i[1] ==-1 and i[2] ==-1:
            ...
            split_0_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "0_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==-1 and i[1] ==-1 and i[2] ==1:
            ...
            split_1_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "1_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==-1 and i[1] ==1 and i[2] ==-1:
            ...
            split_2_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "2_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==-1 and i[1] ==1 and i[2] ==1:
            ...
            split_3_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "3_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==1 and i[1] ==-1 and i[2] ==-1:
            ...
            split_4_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "4_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==1 and i[1] ==-1 and i[2] ==1:
            ...
            split_5_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "5_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==1 and i[1] ==1 and i[2] ==-1:
            ...
            split_6_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "6_y")
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==1 and i[1] ==1 and i[2] ==1:
            ...
            split_7_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar", "7_y")
    np.save(y_path, split_7_y)
    #-----------------------------------------------------------------------------------------
    x_path = os.path.join("..", "data", "splits_cifar", "full_x")
    y_path = os.path.join("..", "data", "splits_cifar", "full_y")
...
def perform_pca_4(keep_digits=[0,1,2,3], pca_dims=32):
    ...
    for index, i in enumerate(train_amplitudes):
        if i[0] ==-1 and i[1] ==-1:
            ...
            split_0_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar_4", "0_y")
    np.save(y_path, split_0_y)
    #-----------------------------------------------------------------------------------------
    split_1_y = []
    for index, i in enumerate(train_amplitudes):
        if i[0] ==-1 and i[1] ==1:
            ...
            split_1_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar_4", "1_y")
    np.save(y_path, split_1_y)
    #-----------------------------------------------------------------------------------------
    split_2_y = []
    for index, i in enumerate(train_amplitudes):
        if i[0] ==1 and i[1] ==-1:
            ...
            split_2_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar_4", "2_y")
    np.save(y_path, split_2_y)
    #-----------------------------------------------------------------------------------------
    split_3_y = []
    for index, i in enumerate(train_amplitudes):
        if i[0] ==1 and i[1] ==1:
            ...
            split_3_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar_4", "3_y")
    np.save(y_path, split_3_y)
    x_path = os.path.join("..", "data", "splits_cifar_4", "full_x")
    y_path = os.path.join("..", "data", "splits_cifar_4", "full_y")
    np.save(x_path, x_train_transformed)
    np.save(y_path, train_amplitudes)
...
def perform_pca_2(keep_digits=[0,1], pca_dims=32):
    ...
    for index, i in enumerate(train_amplitudes):
        if i ==-1:
            ...
            split_0_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar_2", "0_y")
    np.save(y_path, split_0_y)
    #-----------------------------------------------------------------------------------------
    split_1_y = []
    for index, i in enumerate(train_amplitudes):
        if i ==1:
            ...
            split_1_y.append(1)
    y_path = os.path.join("..", "data", "splits_cifar_2", "1_y")
    np.save(y_path, split_1_y)
    x_path = os.path.join("..", "data", "splits_cifar_2", "full_x")
    y_path = os.path.join("..", "data", "splits_cifar_2", "full_y")
    np.save(x_path, x_train_transformed)
    np.save(y_path, train_amplitudes)
